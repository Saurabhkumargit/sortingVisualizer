...
void visualize(int x = -1, int y = -1, int z = -1)
{
    ...
    for (int j = 0; j < arrSize; j++)
    {
        ...
        {
            SDL_SetRenderDrawColor(renderer, 231, 76, 60, 255); // red/orange
        }
        else if (j == y)
        {
            SDL_SetRenderDrawColor(renderer, 155, 89, 182, 255); // purple
        }
        else
        {
            SDL_SetRenderDrawColor(renderer, 149, 165, 166, 255); // light gray
        }
        SDL_RenderFillRect(renderer, &rect);

        // === Draw black outline ===
        SDL_SetRenderDrawColor(renderer, 40, 40, 40, 255); // dark outline
